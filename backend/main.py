...
@app.post("/medicine")
async def create_medicine(payload:Medicine_InPydantic): # type: ignore
...
@app.put("/medicine")
async def update_medicine(user_payload: Medicine_InPydantic): # type: ignore
    data = user_payload.model_dump(exclude_unset=True)

    stored_meds = await Medicine.filter(
        medicine_name=data.get("medicine_name"),
        store_id=data.get("store_id")
    ).first()

    if not stored_meds:
        raise HTTPException(status_code=404, detail="No such medicine exists in this store")
